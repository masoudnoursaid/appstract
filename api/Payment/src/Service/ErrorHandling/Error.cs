...
public readonly record struct Error
{
    ...
    {
        var backendErrorType = ErrorTypeHelper.GetBackendErrorType(code);
        return new Error(Convert.ToInt32(code), backendErrorType, values);
    }

    public static Error FromCommonErrorCode(HandlerCode handlerCode, CommonErrorCode commonCode,
        Dictionary<string, string>? values = null)
    {
        var backendErrorType = ErrorTypeHelper.GetBackendErrorType(commonCode);
