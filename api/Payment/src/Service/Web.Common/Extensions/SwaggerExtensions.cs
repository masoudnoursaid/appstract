...
public static class SwaggerExtensions
{
        ...
        IApiDescriptionGroupCollectionProvider apiDescriptionGroupCollectionProvider)
    {
        app.UseWhen(context => context.Request.Path.StartsWithSegments($"/{SWAGGER_PREFIX_PATH}"),
            appBuilder => { appBuilder.UseMiddleware<SwaggerBasicAuthMiddleware>(); });

        app.UseSwagger(options => { options.RouteTemplate = $"{SWAGGER_PREFIX_PATH}/{{documentName}}/swagger.json"; });
        app.UseSwaggerUI(options =>
        {
            options.RoutePrefix = SWAGGER_PREFIX_PATH;

            foreach (var groupName in apiDescriptionGroupCollectionProvider.ApiDescriptionGroups.Items.Select(a =>
                         a.GroupName!))
